...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of NIKA up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max NIKA entry is capped by minimum between your average NIKA balance in the iNIKA, or the pool’s hard cap. To increase the max entry, Stake more NIKA into the iNIKA',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max NIKA entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iNIKA.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more NIKA for longer durations to increase the maximum number of NIKA you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max NIKA entry is capped by your average NIKA balance in the iNIKA. To increase the max entry, Stake more NIKA into the iNIKA',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. NIKA entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('NIKA to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
